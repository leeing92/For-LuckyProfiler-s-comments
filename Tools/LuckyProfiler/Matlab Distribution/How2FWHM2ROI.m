...
%     [Gx,Gy]=gradient(SkelImwithout1); % Using Gradient calculating angle.
    SkelNum=sum(SkelImwithout1(:));
    [Locx,Locy]=find(SkelImwithout1);
    rr=zeros(1,SkelNum);
    LineProfile=cell(1,SkelNum);
    for i=1:SkelNum
%        G1=[Gx(Locx(i),Locy(i)-2),Gx(Locx(i),Locy(i)-1),Gx(Locx(i),Locy(i)),Gx(Locx(i),Locy(i)+1),Gx(Locx(i),Locy(i)+2)];
%        G2=[Gy(Locx(i)-2,Locy(i)),Gy(Locx(i)-1,Locy(i)),Gy(Locx(i),Locy(i)),Gy(Locx(i)+1,Locy(i)),Gy(Locx(i)+2,Locy(i))];
%        G1_TRY=G1(G1~=0);
%        G2_TRY=G2(G2~=0);
%        if  G2_TRY(1)>0 || (G2_TRY(1)<0 && G2(3)==0)
%            G=[-mean(abs(G1_TRY));mean(abs(G2_TRY))];
%        else
%            G=[mean(abs(G1_TRY));mean(abs(G2_TRY))];
%        end
%        F=[1,-G(1)/G(2)];
...
%        figure(3);plot(LineProfile{i});
    ...
    %end-add on 20220405
     ...
     for j=1:SkelNum
%          if size(LineProfile{j}(:),1)>2*rrmax+1
%              break
%          end
%             rradd=(2*rrmax+1-size(LineProfile{j}(:),1))/2;
